        ...
        double f0, double span, unsigned int bin_length)
{
    ...
    for (ffti; ffti < fft_length; ffti++) {
        ...
        // remove comment from line below to add a simulated axion at 17 GHz
        double thefreq = freq_start + ((float)ffti/(float)fft_length)*freq_span;
        if ((thefreq >= 16999.983) && (thefreq <= 17000.017)) {
            //curr_sum += 2e-12;
            ;
        }
...
void Spectrum::rebin(unsigned int npoints)
{
    unsigned int newlength = (unsigned int)floor(length / npoints);
    double *newpower = new double[newlength];
    double *newuncertainty = new double[newlength];

    double highest_power = 0;
    double highest_uncertainty = 0;
    unsigned int newi = 0;
    for (unsigned int i = 0; i < length; i++) {
        highest_power = fmax(highest_power, power[i]);
        highest_uncertainty = fmax(highest_uncertainty, uncertainty[i]);
        if ((i % npoints) == npoints - 1) {
            newpower[newi] = highest_power;
            newuncertainty[newi] = highest_uncertainty;
            highest_power = 0;
