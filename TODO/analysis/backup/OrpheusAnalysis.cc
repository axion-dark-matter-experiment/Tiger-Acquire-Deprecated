...
int main(int argc,char *argv[]) {
    ...
    for(unsigned int onrun=0;onrun<runs.size();onrun++) {
        ...
        thespectrum.chopBins(200, 200, bin_size);
        ...
        remove_background(thespectrum, bin_size);
...
Spectrum getKSVZPowerInSpectrum(double veff,double B,Spectrum &spectrumformat,double Q,double f0)
{
    ...
    for(unsigned int i=0;i<ret.length;i++) {
        ...
        ret.power[i]=getMaxKSVZPower(veff,B,f)*Q*getLorentzian(f0,f,Q);
    }
    return ret;
}
